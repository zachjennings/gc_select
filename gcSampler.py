...
default_means_bim = np.array([[1.03,0.30],[0.75,0.19]])
...
class gcSampler(object):
	...
	def __init__(self,n_walkers,catalog=np.zeros(1),n_colors=2,spatial_bimodality=False,\
		...
		#initialize the likelihood functions, as desired
		...
		if self.radial_profile is None or self.radial_profile=='none':
			self.fg = densityFG.densityFG(max_rad=max_rad,spatial=False,fg_type=fg_type,fg_kde_file=fg_kde_file,center=center,lum=self.lum,\
											faint_mag=self.fg_faint,bright_mag=self.fg_bright)
		...
		self.gc = densityGC.densityGC(radial_profile=self.radial_profile,ellipticity=self.ellipticity,\
			...
			lum_mean=lum_mean,lum_sig=lum_sig)
		...
		if self.completeness == 'full':
			...
			if self.fixed_cov and self.fixed_lum:
				self.gc_complete_norm = self.norm.calc_gc_norm(None,None,None,None,np.array([1.0]))
			...
			if self.fixed_cov and self.fixed_lum:
				self.gc_complete_norm = self.norm.calc_gc_norm(None,None,None,None,None)
			
			self.ln_complete = completeness_obj.QueryComplete(self.data[1])[:,0]

			'''
			#We need to calculate the normalization of the completeness function
			min_mag = 18.
			max_mag = 26.
			grid_size = 10000.
			self.mag_grid = np.linspace(min_mag,max_mag,grid_size)

			#Evaluate the completeness on a grid, store for normalization of
			#GC completeness later
			self.complete_grid = np.exp(completeness_obj.QueryComplete(self.mag_grid)[:,0])
		...
		self.n_dim = self.n_pop + int(self.radial_profile is not None) + \
			2*int(self.ellipticity) + 5*int(not self.fixed_cov)*(self.n_pop) +\
	...
	def thetaUnpack(self,theta):
		...
		if not self.fixed_cov:
			for j in range(self.n_pop):
				...
				cov[j] = np.array([[theta[i+2],theta[i+4]],[theta[i+4],theta[i+3]]])
				i+= 2*self.n_colors + 1
	...
	def lnLike(self,theta):
		...
		if not self.fixed_cov:
			lp_cov = self.lnPriorCov(cov) + self.lnPriorMeans(means)
		else:
			lp_cov = 0.0
		...
		lp = lp_frac + lp_cov + lp_spatial + lp_lum_function# + lp_n_pred
		...
		if self.completeness is not None:
			...
			'''
			bright_complete = self.mag_grid[self.bright_complete_ind]
			mag_dist = stats.norm(loc=lum_mean,scale=lum_sig)
		
			#normalization for completeness = 1 is just the CDF of the magnitude distribution
			self.bright_complete_norm = mag_dist.cdf(bright_complete)
		
			#normalization for faint end is a numerical approx to the integral of d(mag|theta) d(obs|mag) dm
			self.faint_complete_norm = np.sum(self.complete_grid[self.bright_complete_ind:self.faint_complete_ind]*\
					mag_dist.pdf(self.mag_grid[self.bright_complete_ind:self.faint_complete_ind]) * self.mag_grid_delta)
				
			#combine the completeness for FG and GCs to get p(obs|theta)
			self.completeness_norm = fractions[-1]*self.fg_complete_norm + np.sum(fractions[:-1])*(self.bright_complete_norm + self.faint_complete_norm)
			completeness_norm_ln = np.log(self.completeness_norm)
			#completeness_norm_ln = 0.
			'''
		else:
	...
	def lnPriorMeans(self,means):
		...
		#test location of mean terms in means_0
			if  means_0[0] > 1.5 or means_0[0] < 0.2 or 1.0 < means_0[1] or means_0[1] < 0.0:
				lp_means_0 = -np.inf
			else:
				lp_means_0 = 0.0
			return lp_means_0
			...
			if (means_0[0] < 0.9) or (means_0[1] < 0.23) or (1.2 < means_0[0]) or (0.4 < means_0[1]):
				lp_means_0 = -np.inf
			else:
				lp_means_0 = 0.0
		...
		#test location of mean terms in means_1
			if  (means_1[0] > 0.9) or (means_1[1] > 0.23) or (0.3 > means_1[0]) or (0.05 > means_1[1]):
				lp_means_1 = -np.inf
			else:
				lp_means_1 = 0.0
	...
	def calcStats(self,print_stats=True,print_pgcs=False,high_range=84,low_range=16,n_burn=0.):
		...
		if print_stats:
			param_labels = self.getParamLabels()
			for i in np.arange(self.n_dim):
				print param_labels[i]+' median, ',str(low_range),'%, and ',str(high_range),'% ',self.medians[i],'  ',\
					self.lows[i],'  ',self.highs[i]
			if self.mock:
				print "Number of False Positives: ",self.p_gc[self.false_pos].size
				print "Number of False Negatives: ",self.p_gc[self.false_neg].size
